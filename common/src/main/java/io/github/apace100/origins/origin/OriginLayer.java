...
public class OriginLayer implements Comparable<OriginLayer> {
    ...
    public List<Identifier> getOrigins() {
        ...
        return conditionedOrigins.stream().flatMap(co -> co.getOrigins().stream()).filter(OriginRegistry::contains).distinct().collect(Collectors.toList());
    }
    ...
    public List<Identifier> getRandomOrigins(PlayerEntity playerEntity) {
        return conditionedOrigins.stream().filter(co -> co.isConditionFulfilled(playerEntity)).flatMap(co -> co.getOrigins().stream()).filter(OriginRegistry::contains).filter(o -> !originsExcludedFromRandom.contains(o)).filter(id -> doesRandomAllowUnchoosable || OriginRegistry.get(id).isChoosable()).distinct().collect(Collectors.toList());
    }
