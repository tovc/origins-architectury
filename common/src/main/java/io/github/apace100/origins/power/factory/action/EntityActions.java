...
public class EntityActions {
	...
	public static void register() {
		...
		register(new ActionFactory<>(Origins.identifier("choice"), new SerializableData()
				...
				(data, entity) -> {
					FilterableWeightedList<ActionFactory<Entity>.Instance> actionList = data.get("actions");
				...
				(data, entity) -> {
					...
					if (server != null) {
						ServerCommandSource source = new ServerCommandSource(
								...
								null);
						server.getCommandManager().execute(source, "execute as " + entity.getUuidAsString() + " run " + data.get("command"));
					}
				}));
